...
void MainWindow::encodeMsg()
{
    ...
    QByteArray obraz = text.toUtf8();
    int bytes_size = obraz.size();
    image_size = (image.width() * image.height() * 3) / 8 - marker.size()-message_bytes;
...
void MainWindow::decodeMsg()
{
        ...
        int all_size = 0;
        for (int i = marker.size(); i < marker.size() + message_bytes; ++i) {
